...
void call4___::z__(int& err, deque<string>* ret, const char*& ret2) {
	void* qu = l4_.new_qu__(main_qu_);
	{
		...
		string s;
		s+=dy;s+=ky;s+="AZ/";
		sprintf(buf, "%lx", (unsigned long)my__);s+=buf;
		s+="/&-&-$11-&";s+=by;s+=d;
		s+=ky; sprintf(buf, "%lx", (unsigned long)ret);s+=buf; s+=by;s+=d;
		s+=ky; sprintf(buf, "%lx", (unsigned long)this);s+=buf; s+=by;s+=d;
	...
	if(!err) {
		//cout << **src << " [" << argc << endl;
		ret2 = l4_.callback3__(&err, NULL, **src, false, NULL, qu, argc, argv, 0);
	}
}
...
static void call4_after__(uv_work_t* req, int status) {
	...
	if(call4->call2_) {
		int err = 0;
		const char* ret2;
		call4->call2_->z__(err, NULL, ret2);
	}
	delete call4;
}
...
static void api__(const FunctionCallbackInfo<Value>& args) {
	...
	if(!init_) {
		const char *path = "/usr/lib/zhscript/", *cmdline = "-S syn-zh -E err-zh";
